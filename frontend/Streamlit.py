        ...
        for message in st.session_state.messages:
            if message["role"] == "assistant":
                ...
                if conversation_id:
                    ...
                    # Assign appropriate feedback type based on mapping
                    if st.session_state.get(f"feedback_{conversation_id}") == "👍":
                        feedback_type = "true_negative" if not is_answerable else "true_positive"
                    else:
                        feedback_type = "false_positive" if not is_answerable else "false_negative"
                    
                    # Pass only conversation_id and feedback_type to handle_feedback
